...
public:
	...
	virtual void RestoreStrings()
	{
		...
		#define MACRO_CONFIG_FLOAT(Name,ScriptName,def,min,max,flags,desc)        // nop
	...
	virtual void Save()
	{
		if(!m_pStorage)
			return;
		m_ConfigFile = m_pStorage->OpenFile("mkrace.cfg", IOFLAG_WRITE, IStorage::TYPE_SAVE);
		...
		#define MACRO_CONFIG_FLOAT(Name,ScriptName,def,min,max,flags,desc) if((flags)&CFGFLAG_SAVE){ str_format(aLineBuf, sizeof(aLineBuf), "%s %f", #ScriptName, g_Config.m_##Name); WriteLine(aLineBuf); }
