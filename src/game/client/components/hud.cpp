...
void CHud::RenderCursor()
{
	...
	vec2 ProjStartPos = m_pClient->m_LocalCharacterPos+vel*21;
	vec2 ProjBouncePos = ProjStartPos + vel * m_pClient->m_Tuning.m_LaserReach;
	
	vec2 ms_pos = CalcPos(ProjStartPos, vel, 0, 5000, 500);
	
	if (g_Config.m_ClAimline>0)
		if(weap==WEAPON_RIFLE || (m_pClient->IsDDRaceServer() && weap==WEAPON_SHOTGUN))
		{
			Graphics()->TextureSet(-1);
			Graphics()->LinesBegin();
		...
		{		
			...
			vec2 pos = CalcPos(ProjStartPos, vel, curvature, speed, i);		
