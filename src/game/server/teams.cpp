...
bool CGameTeams::SetCharacterTeam(int ClientID, int Team)
{
	...
	if (Team != TEAM_SUPER && m_TeamState[Team] > TEAMSTATE_OPEN)
		return false;
    if(Team == TEAM_FLOCK && GetTeamState(m_Core.Team(ClientID)) == TEAMSTATE_EMPTY)
        return false;
	...
	if (g_Config.m_SvHideScore)
		...
		GameServer()->SendChat(-1, CGameContext::CHAT_ALL, aBuf);
			if(g_Config.m_SvLaserScore && (pData->m_BestTime == 0))
				GameServer()->CreateLolText(Player->GetCharacter(), false, vec2(0,-75), vec2 (0,-1), 100, "Finish!");
		}
		...
		if (g_Config.m_SvHideScore)
			...
			GameServer()->SendChat(-1, CGameContext::CHAT_ALL, aBuf);
				...
				if(g_Config.m_SvLaserScore)
				{
					...
					GameServer()->CreateLolText(Player->GetCharacter(), false, vec2(0,-75), vec2 (0,-1), 100, aBuf);
				}
			}
	}
	else if (pData->m_BestTime != 0) // tee has already finished?
	{
		if (fabs(time - pData->m_BestTime) <= 0.005)
		{
			GameServer()->SendChatTarget(Player->GetCID(),
				...
				if(g_Config.m_SvLaserScore)
					GameServer()->CreateLolText(Player->GetCharacter(), false, vec2(0,-75), vec2 (0,-1), 100, "+0");
		}
		else
		{
			str_format(aBuf, sizeof(aBuf),
